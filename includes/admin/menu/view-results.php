...
<div class="educare_post imoport">
    <h1>View Results</h1>
    <blockquote>There are lots of options to find students results. You can view results by Class, Exam, Year. And short results by Asc or Decs. For this, you need to change that options. Also, You can view all the results at a same times. For view all results, keep entire options by default and click View Results button. That's it!</blockquote>
...
<form class="add_results" action="<?php echo esc_url($_SERVER['REQUEST_URI']); ?>" method="post">
	...
	<script>
		function select_Table() {
			...
			if (x == 'Year') {
				document.getElementById("select_data").disabled = '';
				document.getElementById("select_data").innerHTML = '<?php educare_get_options('Year', $data);?>';
			}
			*/
		}
...
?>
    ...
    let options = {
       ...
       numberPerPage:<?php echo esc_attr($results_per_page);?>, // How many content per page
        goBar:true, // anable or disable go button
        pageCounter:true, // count page based on numberPerPage
    };

    let filterOptions = {
        el:'#searchBox' // filter or search specific content
    };
